...
#endif
private func getCTime() -> CTimeSpec? {
	var time = CTimeSpec(tv_sec: 0, tv_nsec: 0)
	var retval: CInt = -1
#if os(Linux)
	// use CLOCK_MONOTONIC as system clock
	retval = clock_gettime(CLOCK_REALTIME, &time)
#elseif os(OSX) || os(iOS) || os(watchOS) || os(tvOS)
	...
	if retval == 0 {
		retval = clock_get_time(clockName, &time)
		_ = mach_port_deallocate(mach_task_self_, clockName)
	}
#endif
	return retval == 0 ? time : nil
}
...
public struct Time: Sendable {
	...
	@inlinable
	public init(unixMilli: Int64) {
		self.init(unixMilli: unixMilli, offset: 0)
	}

	public init(unixMilli: Int64, offset: Int) {
		self.init(unix: unixMilli / 1000,
		          nano: (unixMilli % 1000) * 1_000_000, offset: offset)
	}

	@inlinable
	public init(unixMicro: Int64) {
		self.init(unixMicro: unixMicro, offset: 0)
	}
	...
	public init(unixNano: Int64) {
		self.init(unixNano: unixNano, offset: 0)
	}

	public init(unixNano: Int64, offset: Int) {
	...
	public init(offset: Int) {
		if let time = getCTime() {
			self.init(unix: Int64(time.tv_sec),
					  nano: Int64(time.tv_nsec), offset: offset)
		} else {
	...
	public init(_ date: Date) {
		self.init(date, offset: 0)
	}

	public init(_ date: Date, offset: Int) {
	...
	public func `in`(zone offset: Int) -> Time {
		.init(seconds: seconds, nano: Int64(nanoseconds), offset: offset)
	}
...
extension Time: Equatable, Comparable {
	...
	public func after(_ dur: TimeDuration) -> Self {
		var (sec, _) = dur.signedSeconds.addingReportingOverflow(seconds)
		var (nsec, _) = dur.signedNanoseconds.addingReportingOverflow(Int64(nanoseconds))
	...
	public func diff(_ other: Self) -> TimeDuration {
		let diffSecs = seconds - other.seconds
		let diffNanos = Int64(nanoseconds - other.nanoseconds)
		return .init(seconds: diffSecs, nanoseconds: diffNanos)
	}
